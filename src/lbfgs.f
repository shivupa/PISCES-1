...
C     ****************
C
      SUBROUTINE LBFGS(N,M,X,FUNC,FFUNC,DFUNC,EPS)
C
      INTEGER N,M,IPRINT(2),IFLAG
      DOUBLE PRECISION X(N),G(N),DIAG(N),W(N*(2*M+1)+2*M)
      DOUBLE PRECISION F,EPS,XTOL,FUNC
      LOGICAL DIAGCO
      EXTERNAL DFUNC,FUNC
C
...
C
C     IF A STEP IS OBTAINED FOR WHICH THE MODIFIED FUNTCION

...
// progress_timer tmr("EvaluateCsixtyPolPot:", verbose);
  ...
  dVec Efield_comb ; Efield_comb.resize(nSites*4+1 );
...
//   else
    eigenvectors[i*halfnSites +Min2]= beta*ratio;
...
//  double* work= new double[lwork];
  ...
  for (int i =0 ; i < halfnSites; ++i){
    find_closed_aoms(i+halfnSites, Min1, Min2, Min3);
    eigenvectors[i*halfnSites +i]= alpha;
    eigenvectors[i*halfnSites+Min1-halfnSites]= beta;
  ...
  if (info != 0) {
   ...
   for (int j =0 ; j < 3*nSites; ++j)
      cout<<"VirtOrbital["<<j<<"]="<<VirtOrbital[j]<<endl;
   
   for (int j =9*nSites ; j < 14*nSites; ++j)
...
//      cout<<"Overlapping["<<j<<"]="<<Overlapping[j]<<endl;
  ...
  //    double B = 0.05 ;
      ...
      int Nocc = 0;
      int Nvir = 0;
      for (Nocc = 0; Nocc < 30 ; ++Nocc)
       for (Nvir=0; Nvir < 30 ; ++Nvir) {
       cout<<" Nocc and Nvir : "<<Nocc<<" "<<Nvir<<endl;
        for (int i =0 ; i < nSites; ++i){
          for (int j =0 ; j < nSites; ++j){
           ...
           //  cout<<" OccOrbital[120*Nocc+i] "<<OccOrbital[120*Nocc+i]<<endl;

             Sij_p =c1*AngleArray[i*3+0]*AngleArray[j*3+0]*OccOrbital[120*Nocc+i]*VirtOrbital[120*Nvir+j]*
                    CalculateOverlapIntegral_S(c1, a1, 1, 0, 0, Site[i*3+0], Site[i*3+1], Site[i*3+2], c2, a2, 1, 0, 0, Site[j*3+0], Site[j*3+1], Site[j*3+2]);
             Sij_p +=c1*AngleArray[i*3+1]*AngleArray[j*3+1]*OccOrbital[120*Nocc+i]*VirtOrbital[120*Nvir+j]*
                    CalculateOverlapIntegral_S(c1, a1, 0, 1, 0, Site[i*3+0], Site[i*3+1], Site[i*3+2], c2, a2, 0, 1, 0, Site[j*3+0], Site[j*3+1], Site[j*3+2]);
             Sij_p +=c1*AngleArray[i*3+2]*AngleArray[j*3+2]*OccOrbital[120*Nocc+i]*VirtOrbital[120*Nvir+j]*
...
*/
        ...
        //   cout<<"eigenvalues["<<Nvir+30<<"] =" <<eigenvalues[Nvir+30]<<endl;
        //   cout<<"eigenvalues["<<Nocc<<"] =" <<eigenvalues[Nocc]<<endl;
           SijTotal[i] += abs(SijEach[i])/(eigenvalues[Nvir+30]-eigenvalues[Nocc]);
           SijEach[i] = 0.0;
